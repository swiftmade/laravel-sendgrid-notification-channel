...
This package makes it easy to send notifications using [SendGrid](link to service) with Laravel 5.5+, 6.x and 7.x

**Note:** Replace ```SendGrid``` ```SendGrid``` ```:author_name``` ```:author_username``` ```:author_website``` ```:author_email``` ```:package_name``` ```:package_description``` ```:style_ci_id``` ```:sensio_labs_id``` with their correct values in [README.md](README.md), [CHANGELOG.md](CHANGELOG.md), [CONTRIBUTING.md](CONTRIBUTING.md), [LICENSE.md](LICENSE.md), [composer.json](composer.json) and other files, then delete this line.
...
- [Installation](#installation)
	- [Setting up the SendGrid service](#setting-up-the-SendGrid-service)
- [Usage](#usage)
